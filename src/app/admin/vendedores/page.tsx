...
export default function VendedoresManagementPage() {
  ...
  const [editRoles, setEditRoles] = useState<string[]>(["seller"]);
  ...
  const handleOpenEdit = (seller: Seller) => {
    ...
    const initialRoles = Array.isArray(seller.roles) && seller.roles.length > 0
      ? seller.roles
      : (seller.role ? [seller.role] : ["seller"]);
    setEditRoles(initialRoles);
    setEditRole(initialRoles[0] || seller.role || "seller");
  ...
  const handleCreateSeller = async (e: React.FormEvent) => {
    ...
    try {
      const headers = await getAuthHeaders();
      const email = currentUserEmail || sessionStorage.getItem('zono_user_email') || 'diego.boveda@gmail.com';
      const finalRoles = newRoles.length > 0 ? newRoles : [newRole || 'seller'];
  ...
  const handleUpdateSeller = async (e: React.FormEvent) => {
    ...
    try {
      const headers = await getAuthHeaders();
      const email = currentUserEmail || sessionStorage.getItem('zono_user_email') || 'diego.boveda@gmail.com';
      const finalRoles = editRoles.length > 0 ? editRoles : [editRole || 'seller'];
      const res = await fetch("/api/admin/vendedores", {
        method: "POST",
        headers,
        body: JSON.stringify({
                    ...
                    >
                      ...
                      <td className="py-3 px-4">
                        <div className="flex flex-wrap gap-1">
                          {(() => {
                            const userRoles = Array.isArray(seller.roles) && seller.roles.length > 0
                              ? seller.roles
                              : [seller.role || "seller"];
                            return userRoles.map(r => {
                              ...
                              if (rLower === 'admin') {
                                return (
                                  <span key={r} className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-purple-50 text-purple-700 border border-purple-200">
                              ...
                              if (rLower === 'fletero') {
                                return (
                                  <span key={r} className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-amber-50 text-amber-700 border border-amber-200">
                                    Fletero
                                  </span>
                                );
                              }
                              if (rLower === 'administracion') {
                                return (
                                  <span key={r} className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-indigo-50 text-indigo-700 border border-indigo-200">
                                    Administración
                                  </span>
                                );
                              }
                              return (
                                <span key={r} className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-emerald-50 text-emerald-700 border border-emerald-200">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-bold text-slate-700">
                    Roles en el Sistema <span className="text-[10px] text-slate-400 font-normal">(Permite asignar más de un rol)</span>
                  </label>
                  <span className="text-[10px] text-slate-500 font-bold">
                    {newRoles.length} {newRoles.length === 1 ? 'rol asignado' : 'roles asignados'}
                  </span>
                </div>
                <div className="flex flex-wrap gap-2 p-2 bg-slate-50 border border-slate-200 rounded-xl">
                  {AVAILABLE_ROLES.map((r) => {
                    const isSelected = newRoles.includes(r.id);
                    return (
                      <button
                        key={r.id}
                        type="button"
            ...
            <form onSubmit={handleUpdateSeller} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-slate-500 font-bold">
                    {editRoles.length} {editRoles.length === 1 ? 'rol asignado' : 'roles asignados'}
                  </span>
                </div>
                <div className="flex flex-wrap gap-2 p-2 bg-slate-50 border border-slate-200 rounded-xl">
                  {AVAILABLE_ROLES.map((r) => {
