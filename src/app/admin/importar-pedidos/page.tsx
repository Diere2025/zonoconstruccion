...
export default function ImportarPedidosPage() {
  ...
  const handleImportOrders = async () => {
    ...
    try {
      ...
      try {
        addLog("  -> Cargando productos, vendedores, localidades, orígenes, medios de pedido, métodos de pago y líneas telefónicas en paralelo...");
        ...
        dbProducts = results[0];
        dbSellers = results[1];
        dbLocalities = results[2];
        dbAdvSources = results[3];
        dbOrderMediums = results[4];
        dbPaymentMethods = results[5];
        dbPhoneLines = results[6];
        ...
        addLog(`  ✅ Líneas telefónicas cargadas: ${dbPhoneLines?.length || 0}`);
      } catch (err: any) {
      ...
      const existingOrdersMap = new Map<string, { id: string; status: string; delivery_detail?: string }>();
      let page = 0;
      const pageSize = 1000;
      let hasMore = true;
      ...
      while (hasMore) {
        ...
        const { data, error } = await supabase
          .from('orders')
          .select('id, legacy_code, status, delivery_detail')
      ...
      for (const sheet of sheets) {
        ...
        const targetRows = rows.filter((row, idx) => {
          ...
          if (sheet.isCentralSheet) {
            ...
            const isWholesaleCode = orderCode.toUpperCase().startsWith("AQU") || orderCode.toUpperCase().startsWith("POW");
        ...
        for (const row of targetRows) {
          ...
          try {
            ...
            if (activeStatuses.includes(dbOrder.status)) {
              ...
              else if (normStatus === 'entregando') newStatus = 'Pendiente';
          ...
          // Resolve core system medium from spreadsheet description
          let resolvedMediumName = "Otro";
          const normRawMedium = normalizeText(rawMedium);
          if (normRawMedium.includes("whatsapp business api")) {
          ...
          if (existingClient) {
            ...
            // Mark existing client as wholesale if this is an AQU or POW wholesale order
            const isWholesaleCode = orderCode.toUpperCase().startsWith("AQU") || orderCode.toUpperCase().startsWith("POW");
            ...
            const { data: newClient, error: errNc } = await supabase
              ...
              .insert({
                ...
                credit_limit: 0,
                is_wholesale: orderCode.toUpperCase().startsWith("AQU") || orderCode.toUpperCase().startsWith("POW")
              })
              .select('id')
              .single();
          ...
          else if (normStatus === 'entregando') dbOrderStatus = 'Pendiente';
          ...
          if (orderCode.toUpperCase().startsWith("CAMB")) {
            ...
            })(), 60000, "Tiempo de espera agotado en operaciones de base de datos (60s)");
          } catch (err) {
      ...
      <div className="bg-white p-8 rounded-[2rem] shadow-xl border border-slate-100">
        ...
        <div className="mb-6">
          ...
          <div className={cn("bg-white p-5 rounded-2xl border-2 border-emerald-500 shadow-md flex flex-col md:flex-row justify-between items-start md:items-center gap-4 transition-opacity", !importAquafort && "opacity-60")}>
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-1">
                <span className="block text-sm font-extrabold uppercase tracking-wide text-emerald-600">Pedidos Mayoristas (AQU)</span>
              ...
              <span className="block text-xs text-slate-500 font-bold truncate">Docs ID: 1nz545_xNUgdI2LMAGIDCjh6Qs8-vUDHdynzj7jU2wm0 (Filtrado por AQU)</span>
            </div>
            <div className="flex items-center gap-4 self-stretch md:self-auto justify-between md:justify-end border-t md:border-t-0 pt-3 md:pt-0 border-slate-100">
              <a
                href="https://docs.google.com/spreadsheets/d/1nz545_xNUgdI2LMAGIDCjh6Qs8-vUDHdynzj7jU2wm0/edit?gid=786380854"
