...
export default function EstadoResultadosView() {
  ...
  const renderMatrixTable = (isModal = false) => {
    if (!matrix) return null;
    return (
      <table className="w-full text-left text-xs whitespace-nowrap">
        <thead className="bg-slate-50/95 dark:bg-slate-800/95 backdrop-blur-xs text-slate-500 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-20 shadow-xs">
          <tr>
            <th className="py-3 px-2.5 sm:px-4 sticky left-0 top-0 bg-slate-100 dark:bg-slate-800 z-30 w-[170px] sm:w-[240px] min-w-[170px] sm:min-w-[240px] max-w-[170px] sm:max-w-none border-r border-slate-200 dark:border-slate-700 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] font-bold text-slate-700 dark:text-slate-200">
              Rubro / Concepto
            </th>
            <th className="py-3 px-2.5 sm:px-4 text-right min-w-[105px] sm:min-w-[130px] font-bold">Total Mes</th>
            <th className="py-3 px-2 sm:px-3 text-right min-w-[70px] sm:min-w-[85px] font-bold">% Inc.</th>
  ...
  const renderAbsorptionKpis = () => {
    if (!activeSurchargeAnalysis) return null;
    return (
  ...
  const renderAbsorptionDailyTable = (isModal = false) => {
    if (!activeSurchargeAnalysis?.dailyTimeline) return null;
    return (
      <table className="w-full text-left text-xs whitespace-nowrap">
        <thead className="bg-slate-50/95 dark:bg-slate-800/95 backdrop-blur-xs text-slate-500 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-10 shadow-xs">
          <tr>
            <th className="py-2.5 px-3 font-semibold">Día</th>
            <th className="py-2.5 px-3 text-right font-semibold">Costo MP</th>
            <th className="py-2.5 px-3 text-right font-semibold">Recargo Cliente</th>
            <th className="py-2.5 px-3 text-right font-semibold">Saldo (+Superávit / -Absorbido)</th>
            <th className="py-2.5 px-3 text-right font-semibold">% Cobertura</th>
            <th className="py-2.5 px-3 text-center font-semibold">Diagnóstico</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
          {activeSurchargeAnalysis.dailyTimeline.map((item, idx) => {
            ...
            return (
              <tr key={idx} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition">
                <td className="py-2.5 px-3 font-bold text-slate-700 dark:text-slate-200">{item.day}</td>
                <td className="py-2.5 px-3 text-right text-rose-600 font-semibold">{formatCurrency(item.mpCost)}</td>
                <td className="py-2.5 px-3 text-right text-indigo-600 dark:text-indigo-400 font-semibold">{formatCurrency(item.clientSurcharge)}</td>
                ...
                <td className="py-2.5 px-3 text-right text-slate-600 dark:text-slate-300 font-medium">
                  {item.coveragePct}%
                </td>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm overflow-hidden">
        ...
        <div className="p-4 sm:p-5 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => setFullscreenModal('matrix')}
              className="px-3 py-1.5 rounded-xl text-xs font-semibold text-slate-700 dark:text-slate-200 hover:text-indigo-600 dark:hover:text-indigo-400 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200/80 dark:hover:bg-slate-700/80 border border-slate-200/80 dark:border-slate-700 transition flex items-center gap-1.5 shadow-2xs"
              title="Abrir matriz en pantalla completa"
            >
              <Maximize2 className="w-3.5 h-3.5" />
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm overflow-hidden p-6 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800/80 pb-5">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                onClick={() => setFullscreenModal('absorption')}
                className="px-3 py-2 rounded-xl text-xs font-semibold text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950/50 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 border border-indigo-200/80 dark:border-indigo-800/80 transition flex items-center gap-1.5 shadow-2xs"
                title="Abrir auditoría de absorción en pantalla completa"
              >
                <Maximize2 className="w-3.5 h-3.5" />
                <span className="hidden sm:inline">Pantalla Completa</span>
              </button>
          ...
          {showAbsorptionAudit && activeSurchargeAnalysis?.dailyTimeline && (
            <div className="mt-5 pt-4 border-t border-slate-200/80 dark:border-slate-800">
              ...
              <div className="overflow-x-auto max-h-80 overflow-y-auto border border-slate-100 dark:border-slate-800/80 rounded-xl">
                {renderAbsorptionDailyTable(false)}
              </div>
            </div>
          )}
        </div>
      ...
      {/* Fullscreen Modal: Matriz EERR */}
      {mounted && fullscreenModal === 'matrix' && createPortal(
        <div
          className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex flex-col p-2 sm:p-4 md:p-6 animate-in fade-in duration-150"
          onClick={e => {
            if (e.target === e.currentTarget) setFullscreenModal(null);
          }}
        >
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-2xl flex flex-col w-full h-full max-h-full overflow-hidden">
            {/* Modal Toolbar / Header */}
            <div className="p-4 sm:p-5 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 shrink-0 bg-white dark:bg-slate-900">
              <div className="flex flex-wrap items-center gap-3">
                <div className="flex items-center gap-2">
                  <h3 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white">
                    Matriz de Estado de Resultados (EERR)
                  </h3>
                  <span className="text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 border border-indigo-200/50 dark:border-indigo-800/50">
                    Pantalla Completa
                  </span>
                </div>
                ...
                <div className="relative w-48 sm:w-64">
                  ...
                  <input
                    ...
                    onChange={e => setSearchQuery(e.target.value)}
                    placeholder="Filtrar concepto o rubro..."
                    className="w-full pl-9 pr-4 py-1.5 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-800 dark:text-slate-200 focus:outline-hidden focus:ring-2 focus:ring-indigo-500 transition"
                  />
                </div>
                ...
                <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl border border-slate-200/80 dark:border-slate-700 text-xs">
                  <button
                    onClick={expandAll}
                    className="px-2.5 py-1 text-slate-600 dark:text-slate-300 hover:text-indigo-600 rounded-lg font-medium transition flex items-center gap-1"
                    title="Expandir todos los rubros"
                  >
                    <Maximize2 className="w-3.5 h-3.5" />
                    <span>Expandir</span>
              ...
              <div className="flex items-center justify-between sm:justify-end gap-3">
                ...
                <button
                  ...
                  className="p-2 sm:px-3 sm:py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer"
      ...
      {/* Fullscreen Modal: Control de Absorción */}
      {mounted && fullscreenModal === 'absorption' && activeSurchargeAnalysis && createPortal(
        <div
          className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex flex-col p-2 sm:p-4 md:p-6 animate-in fade-in duration-150"
          onClick={e => {
            if (e.target === e.currentTarget) setFullscreenModal(null);
        ...
        >
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-2xl flex flex-col w-full h-full max-h-full overflow-hidden p-4 sm:p-6 gap-5">
            {/* Modal Header */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800/80 pb-4 shrink-0">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <div className="p-2 rounded-xl bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400">
                    <Scale className="w-5 h-5" />
                  ...
                  <span className="hidden md:inline-flex text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 border border-indigo-200/50 dark:border-indigo-800/50">
                    Pantalla Completa
                  </span>
                </div>
                <p className="text-xs text-slate-500">
              ...
              <div className="flex flex-wrap items-center gap-2.5">
                ...
                <button
                  onClick={() => setFullscreenModal(null)}
                  className="p-2 sm:px-3 sm:py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-semibold flex items-center gap-1.5 transition ml-auto sm:ml-0 cursor-pointer"
                  title="Cerrar pantalla completa (ESC)"
                >
                  <X className="w-4 h-4" />
                  <span className="hidden sm:inline">Cerrar</span>
            ...
            {/* Daily Audit Table (Fills remaining height) */}
            <div className="flex-1 flex flex-col min-h-0 pt-2 border-t border-slate-100 dark:border-slate-800">
              <div className="flex items-center justify-between mb-2.5 shrink-0">
                <span className="text-xs font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                  <FileSpreadsheet className="w-3.5 h-3.5 text-indigo-500" />
                  Auditoría Diaria: MercadoPago vs Recargos de Tarjeta ({activeSurchargeAnalysis.dailyTimeline.filter(i => i.mpCost > 0 || i.clientSurcharge > 0).length} días)
                </span>
                <span className="text-[11px] text-slate-400">
                  Criterio: {surchargeCriteria === 'delivery_date' ? 'Fecha de Entrega Programada' : 'Fecha de Carga del Pedido'}
                </span>
              </div>
              <div className="flex-1 overflow-auto min-h-0 border border-slate-200/80 dark:border-slate-800 rounded-2xl bg-white dark:bg-slate-900 shadow-inner">
                {renderAbsorptionDailyTable(true)}
              </div>
            </div>
          </div>
        </div>,
