-- Migration V59: Check for existing Reserva Pedido transaction before generating Entrega
...
BEGIN
  ...
  IF NEW.status IN ('Entregando', 'Entregado') THEN
    -- Only insert 'Entrega' transactions if the order has reservation transactions (meaning it was created in the system)
    IF EXISTS (
      SELECT 1 FROM public.inventory_transactions 
      WHERE reference_id = NEW.id AND type = 'Reserva Pedido'
    ) THEN
